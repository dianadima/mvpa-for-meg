function results = svm_decode_holdout (train, test, train_labels, test_labels, varargin)
...
parse(p, varargin{:});
svm_par = p.Results; %kfold parameter is included here (default) but will be ignored
clear p svm_results;
...
SVM_model = fitcsvm(train, train_labels, 'KernelFunction', svm_par.kernel, 'BoxConstraint', 1,  'Standardize', svm_par.standardize); %train
[pred,scores] = predict(SVM_model, test); %test
