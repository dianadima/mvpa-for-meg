function [ rand_accuracy ] = time_resolved_kfold_rand( data, labels, num_iterations, varargin )
...
%                        If you want to subselect features on source space data, you need to manually provide numerical indices (i.e. 'channels', [1:1000]).
...
rand_accuracy = zeros(num_iterations, floor(size(data,2)/dec_args.window_length));
orig_labels = labels;

for iter = 1:num_iterations
    
    labels = orig_labels(randperm(length(orig_labels)));
    ...
    rand_accuracy(iter,:) = mean(accuracy(1,:,:),2);
